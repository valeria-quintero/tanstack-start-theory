...
const SkillCard = ({ name }: SkillCardProps) => {
    ...
    return ( 
        <article className="feature-card island-shell rise-in rounded-3xl p-5">
            ...
            <button className="inline-flex size-11 items-center justify-center rounded-full border border-(--lines) bg-(--surface-strong) text-(--sea-ink) shadow-md" onClick={() => setLiked((current) => !current)} type="button">
                <Heart className={liked ? 'fill-current text-(--lagoon-dep)' : ''} size={18} />
            </button>
        </article>
     );
}
